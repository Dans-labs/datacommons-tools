...
export default function Menu() {
  ...
  return (
    <div className="bg-indigo-950 text-white w-12 sm:w-70 fixed top-0 left-0 bottom-0 h-screen z-60">
      <div className="flex flex-col gap-4 h-full justify-between">
        <div className="flex flex-col"> 
          <div className="w-full px-2 py-4 sm:px-8">
            <Link to="/">
              <img src={logoWhite} className="hidden sm:block" alt="Logo" />
              <img src={emblemWhite} className="sm:hidden" alt="Logo" />
            </Link>
          </div>
          ...
          <div className="border-t border-white/20 p-1 pt-3 sm:p-4">
            ...
            <Tooltip text={auth.isAuthenticated ? "Logout" : "Login"} pos="right" className="sm:hidden" fullWidth>
              {auth.isAuthenticated ? 
...
function MenuItem({ to, icon, text }: { to: string, icon: ReactElement<{ className?: string }>, text: string }) {
  const styledIcon = icon
    ? cloneElement(icon, { className: "w-6 h-6 sm:mr-2" })
    : null;
  return (
    <li className="w-full">
      <Tooltip text={text} pos="right" className="sm:hidden" fullWidth>
        <Link 
          to={to} 
          className="font-bold px-2 sm:px-4 py-2 rounded-lg hover:bg-black/10 flex items-center w-full" 
