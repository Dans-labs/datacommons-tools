...
export default function Menu() {
  ...
  return (
    <div
      className={`bg-indigo-950 text-white ${expanded ? "w-68" : "w-12"} md:w-70 fixed top-0 left-0 bottom-0 h-screen z-60 transition-all`}
    >
        ...
        <div className="flex flex-col">
          <header className={`w-full ${expanded ? "px-4" : "px-2"} py-4 md:px-6`}>
            <Link to="/">
              <img
                src={logoWhite}
                className={`${!expanded ? "hidden" : " w-full"} md:block`}
                alt="Logo"
              />
              <img
                src={emblemWhite}
                className={`${expanded ? "hidden" : ""} md:hidden w-full max-h-10`}
                alt="Logo"
              />
            </Link>
          </header>
          ...
          <NavigationMenu.Root orientation="vertical">
            <NavigationMenu.List
              className={`flex flex-col gap-1 ${expanded ? "p-2" : "p-1"} sm:p-4 mb-2`}
            >
              <MenuItem
                to="/"
                text={m.explore_tools()}
          ...
          <div className={`${expanded ? "px-2" : "px-1"} sm:px-4`}>
            <AuthButtons expanded={expanded} className="w-full" />
          </div>
        </div>
...
function AuthMenu({ expanded }: { expanded: boolean }) {
  ...
  return (
    <NavigationMenu.List className={`flex flex-col gap-1 ${expanded ? "p-2" : "p-1"} sm:p-4`}>
      <MenuItem
        to="/tools/my-tools"
        text={m.my_tools()}
        icon={<WrenchIcon />}
